    ...
    <form class="swap-card" @submit.prevent>
        ...
        <div class="flex w-full items-center">
            ...
            <button class="focus:outline-none" @click="invertTokens">
                <svg xmlns="http://www.w3.org/2000/svg"
                     class="w-12 h-12 p-2 bg-white dark:bg-violet-500 border border-violet-200 rounded-full"
                     viewBox="0 0 512 512">
        ...
        <div class="relative grid grid-cols-[minmax(max-content,100px),minmax(auto,1fr)] gap-4">
            ...
            <p class="col-span-2 text-xs flex items-center gap-2">
                Pool price :
                <small-tag v-if="currentPool?.price">
                    {{ `1 ${currentPool?.tokenA} = ${currentPool?.price} ${currentPool?.tokenB}` }}
                </small-tag>
...
import ConnectMetaMask from '../metamask/ConnectMetaMask.vue';
